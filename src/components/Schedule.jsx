...
const LeafletMap = dynamic(
  () => import("./LeafletMap").then((mod) => mod.LeafletMap),
  {
    ssr: false,
    loading: () => (
      <div className="flex items-center justify-center h-full dk-hint">
        Loading map...
      </div>
    ),
  }
  ...
  return (
    <div className="grid auto-rows-min gap-1 p-4 md:px-6 bg-white">
      <div className="flex items-center gap-2">
        <div className="text-[13px] font-semibold text-[#1d70b8] tabular-nums whitespace-nowrap">
          {displayTime}
        </div>
...
const DayGroup = ({ date, events }) => {
  ...
  let dayLabel = format(dateObj, "EEEE, MMMM d, yyyy");

  if (isToday(dateObj)) {
    dayLabel = `Today - ${format(dateObj, "MMMM d, yyyy")}`;
  } else if (isTomorrow(dateObj)) {
    dayLabel = `Tomorrow - ${format(dateObj, "MMMM d, yyyy")}`;
  } else if (isYesterday(dateObj)) {
    dayLabel = `Yesterday - ${format(dateObj, "MMMM d, yyyy")}`;
  }

  return (
    <div>
      <div className="px-4 py-2 bg-[#f3f2f1] border-y border-[#e5e6e7] font-semibold text-[13px]">
        {dayLabel}
      </div>
    ...
    <main>
      ...
      <div className="scrollarea">
        <div className="p-4 bg-white">
          ...
          {!locationLoading && !locationError && !location && (
            <div className="dk-hint">No location data available</div>
          )}
        </div>
        <hr />
        <div className="p-4 bg-white">
